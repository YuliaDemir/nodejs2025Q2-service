...
export class FavoritesService {
  findAll(): { artists: Artist[]; albums: Album[]; tracks: Track[] } {
    return {
      artists: Array.from(db.favorites.artists)
        .map((artistId) => db.artists.find((artist) => artist?.id === artistId))
        .filter((a): a is Artist => a !== undefined),
      albums: Array.from(db.favorites.albums)
        .map((albumId) => db.albums.find((album) => album?.id === albumId))
        .filter((a): a is Album => a !== undefined),
      tracks: Array.from(db.favorites.tracks)
